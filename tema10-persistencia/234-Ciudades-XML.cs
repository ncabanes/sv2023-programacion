/* 234. Crea una nueva versión del ejercicio 210 (ciudades + ciudades), partiendo
 * de la versión oficial, en la que uses serialización XML para cargar datos
 * cuando arranca el programa y para guardar los datos tras cada modificación.
 * 
 * VICTOR (...) - 1ºDAM SEMI */
...
class PruebaCiudad
{
    ...
    static void Guardar(List<Ciudad> ciudades)
    {
        try
        {
            XmlSerializer formatter = new XmlSerializer(ciudades.GetType());
            Stream stream = new FileStream("ciudades.xml",
                FileMode.Create, FileAccess.Write, FileShare.None);
            formatter.Serialize(stream, ciudades);
            stream.Close();
        }
    ...
    public static List<Ciudad> Cargar()
    {
        ...
        {
            XmlSerializer formatter = new XmlSerializer(ciudades.GetType());
            FileStream stream = new FileStream("ciudades.xml",
                FileMode.Open, FileAccess.Read, FileShare.Read);
            ciudades = (List<Ciudad>)formatter.Deserialize(stream);
            stream.Close();
            return ciudades;
        }
        catch (PathTooLongException)
