/* 210. Crea una versión ampliada del ejercicio 191 (ciudades como 
list+class), partiendo de la versión oficial, en la que guardes datos 
tras cada cambio (añadir, modificar, ordenar, etc). y los cargues al 
comenzar la ejecución. Al igual que en el ejercicio anterior, el 
formato de los datos queda a tu criterio. Recuerda gestionar los 
posibles errores de ficheros de forma adecuada. */
...
class PruebaCiudad
{
    static void Main()
    {
        ...
        do
        {
            ...
            switch (opcion)
            {
                case "1": Anyadir(ciudades); Guardar(ciudades); break;
                case "2": Mostrar(ciudades); break;
                case "3": Buscar(ciudades); break;
                case "4": Modificar(ciudades); Guardar(ciudades); break;
                case "5": Insertar(ciudades); Guardar(ciudades); break;
                case "6": Eliminar(ciudades); Guardar(ciudades); break;
                case "7": Ordenar(ciudades); Guardar(ciudades); break;
    ...
    static void Guardar(List<Ciudad> ciudades)
    {
        try
        {
            StreamWriter f = File.CreateText("ciudades.dat");
            foreach (Ciudad c in ciudades)
            {
                f.WriteLine(c.Nombre + "@" +
                    c.Pais + "@" +
                    (c.Poblacion));
            }
            f.Close();
    ...
    public static List<Ciudad> Cargar()
    {
        ...
        try
        {
            ...
            string linea;
            if (File.Exists("ciudades.dat"))
            {
                StreamReader f = File.OpenText("ciudades.dat");
                do
                {
                    linea = f.ReadLine();
                    if (linea != null)
                    {
                        ...
                        string pais = trozos[1];
                        int poblacion = Convert.ToInt32(trozos[2]);
                        Ciudad t = new Ciudad(nombre, pais, poblacion);
                        lista.Add(t);
                    }
                }
                while (linea != null);
